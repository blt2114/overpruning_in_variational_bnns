...
class model_base:
    ...
    def load_data(self, n_pts=None, bimodal=True):
        ...
        elif self.dataset == 'toy_small':
            n_pts = 500 if n_pts is None else n_pts
            n_pts = int(np.ceil(n_pts/5.))*5
            self.X = np.random.normal(size=[n_pts,1])
            self.Y = np.array([0.1, 0.2, 0.3, 0.9,1.0]*(int(n_pts/5)))
            self.Y = self.Y.reshape([n_pts,1])
        elif self.dataset == 'mog':
            # in the case of MoG, this is very much not random, so data must
            # be shuffled in train test split.
            self.X, self.Y = synthetic_data.gen_mog_synthetic_data()
        elif self.dataset == 'chicago' or self.dataset == 'chicago_small':
            small = self.dataset == 'chicago_small'
            self.X, self.Y, self.feature_idxs = chicago.load_crime(small,
                    crime_type=True)
            self.dataset = 'chicago'
            print "self.X[0]: ", self.X[0]
        elif self.dataset == 'nyc_taxi':
            self.X, self.Y, self.feature_idxs = nyc.load_taxi()
        else:
            self.data_dir = self.data_dir_base +"/"+self.dataset+"/data/"
            data = np.loadtxt(self.data_dir+'data.txt')
            index_features = np.loadtxt(self.data_dir+'index_features.txt',dtype=int)
            index_target = np.loadtxt(self.data_dir+'index_target.txt',dtype=int)
            self.X = data[ : , index_features.tolist() ]
            self.Y = data[ : , index_target.tolist() ]
